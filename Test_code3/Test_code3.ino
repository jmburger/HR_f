...
void loop() {
  ...
  float Filtered_IR_val = 0;
                   //store filtered IR vector values in flash memory
  ...
  {     
    ...
    if (sensor.getRawValues(&raw_IR_Val, &raw_RED_Val))
    {
      ...
      PROGMEM float Filtered_IR_vec[SIZE][2] = { [i; 1], [Filtered_IR_val; 2] };
      //Serial.print(" | ");
